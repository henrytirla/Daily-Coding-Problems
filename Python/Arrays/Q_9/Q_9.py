...
def ValidSudoku(board):
    ...
    square_check = collections.defaultdict(set)

    for r in range(9):
        for c in range(9):
            if board[r][c] ==".":
               continue
            if(board[r][c] in cols[c] or board[r][c] in row[c] or board[r][c] in square_check[r//3,c//3]):
                return False

            cols[r].add(board[r][c])
            row[c].add(board[r][c])
            square_check[r//3,c//3].add(board[r][c])
    return True
