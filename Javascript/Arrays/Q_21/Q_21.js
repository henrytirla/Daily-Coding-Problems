...
function BestSeat(seat){
    let BestPlace= -1
    let maxPlace =0
    let leftIdx = 0

    while(leftIdx< seat.length){
        let rightIdx= leftIdx+1
        while(rightIdx< seat.length && seat[rightIdx]== 0){
            rightIdx++}
        let availableSpace = rightIdx-leftIdx-1
        if (availableSpace> maxPlace){
            maxPlace= availableSpace
            BestPlace = Math.floor((leftIdx + rightIdx) / 2);
        }
        leftIdx =rightIdx
